...
2. Set up the env:
   ...
   ```
3. Download the `tc-filter.py` script.  (or clone the entire braindump, if preferred):
   ...
   ```
5. Configure `TASKCLUSTER_ROOT` to point to mozilla's instance.  Recommend adding the following line to your `.bashrc`:

   ```sh
   export TASKCLUSTER_ROOT_URL=https://firefox-ci-tc.services.mozilla.com
   ```
6. Sign into taskcluster.  This will create a short lived client with your credentials and pass back a token which will be stored in environment variables:

   ```sh
   eval $(taskcluster signin)
   ```
7. Now the bootstrapping is done, and you can rerun tasks as needed. You can get `graph-id` from the taskcluster url. (eg https://firefox-ci-tc.services.mozilla.com/tasks/groups/KRYQ42TsSki2Dc2_OtOHIg -> KRYQ42TsSki2Dc2_OtOHIg):
